...
import { FaUser, FaBuilding, FaEnvelope, FaRegLightbulb, FaHandshake } from "react-icons/fa";
...
export default function SponsorshipPage() {
    ...
    try {
      const res = await fetch("/api/lib/email", {
        ...
        body: JSON.stringify({
          ...
          subject: `New Sponsorship Inquiry from ${form.name}`,
          html: `<b>Name:</b> ${form.name}<br/><b>Company:</b> ${form.company}<br/><b>Email:</b> ${form.email}<br/><b>Motivation:</b> ${form.motivation}<br/><b>Type:</b> ${form.type.join(", ")}<br/><b>Additional:</b> ${form.additional}`,
        }),
      });
      ...
      toast({
        title: "Thank you!",
        description: "Your sponsorship inquiry has been received. We'll be in touch soon!",
        variant: "success",
      });
    } catch {
      setError("There was a problem submitting your sponsorship inquiry. Please try again.");
      toast({
        title: "Submission Failed",
        description: "There was a problem submitting your sponsorship inquiry. Please try again.",
        variant: "destructive",
      });
    } finally {
      setSubmitting(false);
  ...
  function next() {
    if (step === 0 && !isSponsorDetailsValid) {
      setTouched({ name: true, company: true, email: true, motivation: true, type: true });
      return;
    }
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-blue-50 via-blue-100 to-blue-200 flex flex-col items-center justify-center py-12 px-2 sm:px-6">
      <div className="w-full max-w-4xl mx-auto relative">
        ...
        >
          <h1 className="text-3xl sm:text-4xl font-extrabold text-center mb-2 text-[#0B1C39] tracking-tight">Become a <span className="text-blue-700">Sponsor</span></h1>
          <p className="text-center text-gray-600 mb-8 text-lg max-w-2xl mx-auto">Support the Web3 Mates community and help us grow!</p>
          {/* Stepper */}
          ...
          <AnimatePresence mode="wait">
            {success ? (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center flex flex-col items-center justify-center min-h-[300px]">
                ...
                <p className="mb-2">Your sponsorship inquiry has been received.<br />We&apos;ll be in touch soon.</p>
              </motion.div>
            ) : (
              <motion.form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                {step === 0 && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="relative">
                      ...
                      <input name="name" value={form.name} onChange={handleChange} onBlur={handleBlur} required className={`w-full pl-10 pr-3 py-3 border rounded-lg bg-white/80 focus:outline-none focus:border-[#0066FF] text-black ${(touched.name && !nameCompanyPattern.test(form.name.trim())) ? 'border-red-500' : ''}`} placeholder="Full Name" />
                      {touched.name && !nameCompanyPattern.test(form.name.trim()) && <div className="text-red-500 text-xs mt-1">Enter a valid name (letters, spaces, min 2 chars).</div>}
                    </div>
                    <div className="relative">
                      <FaBuilding className="absolute left-3 top-1/2 -translate-y-1/2 text-blue-300 text-lg pointer-events-none" />
                      <input name="company" value={form.company} onChange={handleChange} onBlur={handleBlur} required className={`w-full pl-10 pr-3 py-3 border rounded-lg bg-white/80 focus:outline-none focus:border-[#0066FF] text-black ${(touched.company && !nameCompanyPattern.test(form.company.trim())) ? 'border-red-500' : ''}`} placeholder="Company/Organization" />
                      {touched.company && !nameCompanyPattern.test(form.company.trim()) && <div className="text-red-500 text-xs mt-1">Enter a valid company name (letters, spaces, min 2 chars).</div>}
                    </div>
                ...
                {step === 2 && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    <label className="block mb-2 font-medium text-[#0B1C39]">What would you like to sponsor?</label>
                    ...
                    {touched.type && form.type.length === 0 && <div className="text-red-500 text-xs mb-2">At least one sponsorship type is required.</div>}
                  </motion.div>
                )}
                {step === 3 && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className=" relative">
                    <textarea name="additional" value={form.additional} onChange={handleChange} onBlur={handleBlur} rows={3} className="w-full pl-4 pr-3 py-3 border rounded-lg bg-white/80 focus:outline-none focus:border-[#0066FF] text-black" placeholder="Any additional information? (optional)" />
                  </motion.div>
                )}
                {step === 4 && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    ...
                    <ul className="mb-4 text-sm">
                      ...
                      <li className="text-slate-500"><b>Company:</b> {form.company}</li>
                      <li className="text-slate-500"><b>Email:</b> {form.email}</li>
                      <li className="text-slate-500"><b>Motivation:</b> {form.motivation}</li>
                      <li className="text-slate-500"><b>Sponsorship Type:</b> {form.type.join(", ")}</li>
                      <li className="text-slate-500"><b>Additional Info:</b> {form.additional}</li>
                    </ul>
                  </motion.div>
                )}
